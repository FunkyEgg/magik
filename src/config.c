...
MagikError parseConfig(MagikConfig* config, toml_table_t* toml_data) {
    ...
    config->data.project.name = toml_string_in(project_table, "name");
    if (!config->data.project.name.ok) { return MGK_UNABLE_TO_PARSE_TOML_DATA; }

    config->data.project.ver = toml_string_in(project_table, "version");
